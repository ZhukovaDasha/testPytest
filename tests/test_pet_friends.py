...
def test_post_add_new_pet_with_valid_data(name='Руна', animal_type='собака', age='1', pet_photo='images/IMG_8521.jpeg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Лекси", "кот", "3", "images/cat1.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_successful_update_self_pet_info(name='Юнона', animal_type='Котэ', age=5):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    if len(my_pets['pets']) > 0:
...
def test_post_add_new_pet_without_photo(name='Руна', animal_type='собака', age=12):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
